...
from common.php_converter import php_to_js, convert_php_array_to_json, convert_php_array_with_php_r
...
class DirectiveParsers:
    ...
    def parse_props(self, blade_code):
        """Parse @props directive — supports both standard and array format.
        Standard: @props($a = 0, $c = 0) — same as @vars
        Array: @props(['a' => 'default', 'b' => 0]) — key-value pairs
        JS output is identical to @vars: let {...} = __data__;
        """
        blade_code = self._remove_verbatim_blocks(blade_code)
        
        # Use balanced parentheses to handle nested brackets
        pattern = r'@props\s*\('
        match = re.search(pattern, blade_code)
        if not match:
            return ''
        ...
        props_content = content.strip()
        var_parts = []
        
        # Check for array format: ['key' => value, ...]
        if props_content.startswith('[') and props_content.endswith(']'):
            inner = props_content[1:-1].strip()
            ...
            for part in parts:
                ...
                if '=>' in part:
                    ...
                    value = self._convert_php_to_js(value)
    ...
    def parse_states_directives(self, blade_code):
        """Parse @states directives — supports both array and standard format.
        Array: @states(['count' => 0, 'e' => $b])
        Standard: @states($count = 0, $e = $b)
        Both produce const [key, setKey] = useState(value); for each variable.
        """
        blade_code_filtered = self._remove_script_tags(blade_code)
        blade_code_filtered = self._remove_verbatim_blocks(blade_code_filtered)
        
        # Find @states with balanced parentheses
        pattern = r'@states\s*\('
        declarations = []
        
        for match_obj in re.finditer(pattern, blade_code_filtered):
            start_pos = match_obj.end() - 1
            content, end_pos = extract_balanced_parentheses(blade_code_filtered, start_pos)
            if content is None or not content.strip():
                continue
            
            states_content = content.strip()
            
            if states_content.startswith('[') and states_content.endswith(']'):
                ...
                parts = self._split_vars_content_correct(states_content)
                for part in parts:
                    part = part.strip()
                    equals_pos = self._find_first_equals(part)
                    if equals_pos != -1:
                        key = part[:equals_pos].strip().lstrip('$')
    ...
    def parse_onblock_directives(self, blade_code):
        ...
        def replace_onblock(match):
            ...
            if params_expr.startswith('[') and params_expr.endswith(']'):
                ...
                from common.php_converter import convert_php_array_to_json
    ...
    def _convert_extends_expression(self, view_expr):
        ...
        # Handle variables and concatenation: $theme.'.layout', $abc.'.def'.$Abxy
            ...
            # Has concatenation - emit direct JS expression
