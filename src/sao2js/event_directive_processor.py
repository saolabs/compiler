...
class EventDirectiveProcessor:
    ...
    def process_event_directive(self, event_type, expression):
        ...
        """
        try:
            handler_items = self.process_event_items(expression)
    ...
    def process_event_items(self, expression):
        """Return event handler items (object handlers or arrow functions) in source order."""
        ...
        for part in parts:
            ...
            # Named function call without $ prefix.
            # Only use direct arrow when the function is from useState mechanism.
            call_match = re.match(r'^([a-zA-Z_][a-zA-Z0-9_]*)\s*\((.*)\)$', part, re.DOTALL)
            if call_match:
                func_name = call_match.group(1)
                if self._is_usestate_function_name(func_name):
                    handler_items.append(self._process_expression_to_arrow(part))
                else:
                    handler = self.parse_handler(part)
                    if handler:
                        params_str = ','.join(handler['params'])
                        handler_items.append(f'{{"handler":"{handler["handler"]}","params":[{params_str}]}}')
                continue

            # Named function call with $ prefix.
            dollar_call_match = re.match(r'^\$([a-zA-Z_][a-zA-Z0-9_]*)\s*\((.*)\)$', part, re.DOTALL)
            if dollar_call_match:
                func_name = dollar_call_match.group(1)
                if self._is_usestate_function_name(func_name):
                    handler_items.append(self._process_expression_to_arrow(part))
                else:
                    params_string = dollar_call_match.group(2).strip()
                    params = self.parse_handler_parameters(params_string)
                    params_str = ','.join(params)
                    handler_items.append(f'{{"handler":"{func_name}","params":[{params_str}]}}')
                continue

            # Simple function name (no params).
            simple_match = re.match(r'^([a-zA-Z_][a-zA-Z0-9_]*)$', part)
            if simple_match and not self._is_usestate_function_name(simple_match.group(1)):
                handler_items.append(f'{{"handler":"{simple_match.group(1)}","params":[]}}')
                continue

            # Fallback expression handling.
            expressions = self._split_expressions_by_semicolon(part)
            for expr in expressions:
                handler_items.append(self._process_expression_to_arrow(expr))

        return handler_items

    def _is_usestate_function_name(self, func_name):
        """Check if a function name maps to useState variable/setter naming."""
        if not func_name:
            return False
        if func_name in self.usestate_variables:
            return True
        if func_name.startswith('set') and len(func_name) > 3:
            state_name = func_name[3].lower() + func_name[4:]
            return state_name in self.usestate_variables
        return False
    ...
    def parse_handler_parameters(self, params_string):
        ...
        for param in params:
            ...
            # Nested function calls in params are treated as deferred/evaluated params.
            # If they involve event alias, collapse to @EVENT placeholder.
            if self._is_function_call_in_param(param):
                if re.search(r'@(?:event|Event|EVENT)\b|\$(?:event|Event|EVENT)\b', param):
                    processed_params.append('"@EVENT"')
                    continue

                nested_js = self.convert_php_variable_to_js(param)
        ...
        if match:
            ...
            if func_name in self.usestate_variables:
                ...
                for param in params:
                    ...
                    param_js = param_js.replace('"@EVENT"', 'event')
