...
class BladeCompiler:
    ...
    def compile_blade_to_js(self, blade_code, view_name, function_name=None, factory_function_name=None):
        ...
        if has_children_directive:
            ...
            # Find existing @vars or @props declaration and append, or create new one
            vars_found = False
            for decl in all_declarations:
                if decl.get('type') in ('vars', 'props'):
                    # Check not already present
                    existing_names = [v['name'] for v in decl['variables']]
                    if '__ONE_CHILDREN_CONTENT__' not in existing_names:
                        decl['variables'].append(children_var)
                    vars_found = True
                    break
            if not vars_found:
                # Insert a new @vars declaration at position 0
                all_declarations.insert(0, {
                    'type': 'vars',
                    'variables': [children_var]
                })
        ...
        props_declaration = self.parsers.parse_props(blade_code)

        # Merge @props into vars_declaration for compatibility
        if props_declaration:
            if vars_declaration:
                # Merge: extract inner parts and combine
                import re as _re
                vars_inner = _re.search(r'let\s*\{(.*?)\}\s*=', vars_declaration, _re.DOTALL)
                props_inner = _re.search(r'let\s*\{(.*?)\}\s*=', props_declaration, _re.DOTALL)
                if vars_inner and props_inner:
                    combined = vars_inner.group(1).strip() + ', ' + props_inner.group(1).strip()
                    vars_declaration = f"let {{{combined}}} = $$$DATA$$$ || {{}};"
            else:
                vars_declaration = props_declaration

        # Also update vars_declaration for compatibility (prerender, config checks)
        if has_children_directive:
            if vars_declaration:
                # Append to existing vars: let {existing, __ONE_CHILDREN_CONTENT__ = ''} = ...
                vars_declaration = re.sub(
                    r'(let\s*\{)(.*?)(\}\s*=\s*\$\$\$DATA\$\$\$)',
                    r"\1\2, __ONE_CHILDREN_CONTENT__ = ''\3",
                    vars_declaration
                )
            else:
                vars_declaration = "let {__ONE_CHILDREN_CONTENT__ = ''} = $$$DATA$$$ || {};"
        ...
        states_declarations = self.parsers.parse_states_directives(blade_code)
        
        # Merge @states into usestate_declarations for legacy compatibility
        if states_declarations:
            if usestate_declarations:
                usestate_declarations = usestate_declarations + '\n' + states_declarations
            else:
                usestate_declarations = states_declarations
        ...
        # ========================================================================
        # Process @import directives and resolve custom tags
        # ========================================================================
        ...
        # Step 3: Resolve custom tags (<tagName ...> / <tagName />) to @include directives
        if component_imports:
            tag_resolver = ImportTagResolver(imports=component_imports, target='js')
            blade_code = tag_resolver.resolve_tags(blade_code)
        ...
        # Store blade code for prerender content extraction before @block/@endblock is removed
        blade_code_original_for_prerender = blade_code
        
        sections_info = self.template_analyzer.analyze_sections_info(sections, vars_declaration, has_await, has_fetch, state_variables=usestate_variables, blade_code=blade_code_original_for_prerender)
        ...
        # Calculate has_prerender - check raw blade_code BEFORE template processing
        # so @if/@foreach/@switch directives are still visible
        has_prerender = self._calculate_prerender_need(
            has_await, has_fetch, vars_declaration, 
            sections_info, blade_code,  # Pass raw blade_code, not processed template_content
        ...
        # ========================================================================
        # V2 Structured Render: AST-based render generation with hydrate IDs
        # ========================================================================
        try:
            # Prepare blade_code_for_ast: apply same cleanups as main flow
            # but WITHOUT useBlock/onBlock transforms (keep @block directives intact)
            ast_blade = blade_code_for_ast
            
            # Remove setup/import scripts
            script_types_ast = ['setup', 'import', 'imports', 'scope', 'scoped']
            for st in script_types_ast:
                ast_blade = re.sub(rf'<script\s+{st}[^>]*>.*?</script>', '', ast_blade, flags=re.DOTALL | re.IGNORECASE)
            
            # Remove compiler-only directives
            ast_blade = re.sub(r'@viewtype\s*\([^)]*\)', '', ast_blade, flags=re.IGNORECASE)
            ast_blade = re.sub(r'@await\s*(?:\([^)]*\))?\s*', '', ast_blade, flags=re.IGNORECASE)
            ast_blade = re.sub(r'@fetch\s*\([^)]*\)\s*', '', ast_blade, flags=re.IGNORECASE)
            ...
            # Resolve @import / custom tags
            ast_import_parser = ImportParser()
            ast_imports = ast_import_parser.parse_imports(ast_blade)
            ast_blade = ast_import_parser.remove_imports(ast_blade)
            if ast_imports:
                ast_tag_resolver = ImportTagResolver(imports=ast_imports, target='js')
                ast_blade = ast_tag_resolver.resolve_tags(ast_blade)
            
            # @importInclude...@endImportInclude blocks are now parsed directly by AST parser
            # (no longer flattened to @include)
            
            # Remove @extends directive (already parsed into extended_view/extends_expression)
            ast_blade = re.sub(r"@extends\s*\([^)]*\)", '', ast_blade, flags=re.IGNORECASE)
            
            # Parse AST
            ast_parser = TemplateASTParser(state_variables=usestate_variables)
            ast_root = ast_parser.parse(ast_blade)
            
            # Determine extends info
            has_extends = bool(extended_view or extends_expression)
            ext_expr = None
            if extended_view:
                 ext_expr = f"__layout__ + '{extended_view}'"
            elif extends_expression:
                ext_expr = extends_expression
            
            # Compute which static sections are already handled in prerender
            # so render can skip them (avoid duplicate declarations)
            prerendered_section_names = set()
            if has_prerender and (has_await or has_fetch):
                for si in (sections_info or []):
                    if not si.get('useVars', False):
                        prerendered_section_names.add(si['name'])

            declared_template_variables = self._extract_declared_template_variables(
                variable_list,
                const_declarations,
                usestate_variables
            )

            # Generate structured render body
            ...
            structured_body = render_gen.generate(
                ...
                extends_expression=ext_expr,
                extends_data=extends_data,
                prerendered_sections=prerendered_section_names
            )
            
            # Build structured render function
            indent = '    '
            ...
            import sys as _sys
            print(f"[saola] AST render generation failed, using legacy: {e}", file=_sys.stderr)
        ...
        for section in sections_info:
            ...
            is_prerendered_static = has_prerender and (has_await or has_fetch) and not section.get('useVars', False)
            if section_type == 'long' and not is_prerendered_static:
                section_name = section.get('name', '')
                if section_name:
...
{indented_content}
    ...
    prerenderSections: """ + prerender_sections_json
            ...
            commit_params = 'this: any' if is_typescript else ''
            update_data_params = f"this: any, {data_param}" if is_typescript else data_param
            update_item_params = f"this: any, {key_value_params}" if is_typescript else key_value_params
        ...
        updateVariableItemData: function(""" + update_item_params + """) {
            this.data[key] = value;
...
export function """ + function_name + """(__data__ = {}, systemData = {}) {
    ...
    def _add_typescript_annotations(self, code, class_name, function_name):
        ...
        code = re.sub(
            r'updateVariableData: function\(data: any\)',
            r'updateVariableData: function(this: any, data: any)',
            code
        )
        ...
        code = re.sub(
            r'updateVariableItemData: function\(key: string, value: any\)',
            r'updateVariableItemData: function(this: any, key: string, value: any)',
            code
        )

        # Add this typing to commitConstructorData callback
        code = re.sub(
            r'commitConstructorData: function\(\)',
            r'commitConstructorData: function(this: any)',
            code
        )

        # Add types to generated controller getConfig method
        code = re.sub(
            r'getConfig\(key, defaultValue = null\)',
            r'getConfig(key?: string, defaultValue: any = null): any',
            code
        )
    ...
    def _extract_sections_from_prerender(self, has_prerender, has_await, has_fetch, sections_info):
        ...
        # Include static sections/blocks handled directly in prerender and dynamic ones with preloader.
        for section in sections_info:
            section_name = section.get('name', '')
            if section_name:
                if (has_await or has_fetch) and not section.get('useVars', False):
                    sections_used.append(section_name)
                    continue
    ...
    def _extract_usestate_variables(self, usestate_declarations, all_declarations):
        ...
        for decl in all_declarations:
            if decl.get('type') in ['useState', 'states', 'const', 'let']:
    ...
    def _resolve_import_includes_for_ast(self, code):
        """Resolve @importInclude...@endImportInclude blocks to flat @include directives for AST parsing.
        
        Converts:
            @importInclude(tagName, path, ['attr' => val])
            children_content
            @endImportInclude
        →
            @include(path, ['attr' => val, '__ONE_CHILDREN_CONTENT__' => '...'])
        """
        max_iters = 100
        for _ in range(max_iters):
            match = re.search(r'@importInclude\s*\(', code)
            if not match:
                break
            
            paren_start = match.end() - 1
            args_content, args_end = extract_balanced_parentheses(code, paren_start)
            ...
            children = self._resolve_import_includes_for_ast(children)
            ...
            # Build @include with children merged into data
            if data:
                # Remove trailing ] and add __ONE_CHILDREN_CONTENT__
                data_stripped = data.rstrip()
                if data_stripped.endswith(']'):
                    data = data_stripped[:-1] + "]"
                replacement = f"@include({path}, {data})"
            else:
                replacement = f"@include({path})"
            
            # Replace children as separate @include directives before the parent
            parts = []
            if children.strip():
                parts.append(children)
            parts.append(replacement)
            
            code = code[:match.start()] + '\n'.join(parts) + code[total_end:]
    ...
    def _calculate_prerender_need(self, has_await, has_fetch, vars_declaration, sections_info, raw_blade_code, usestate_declarations, let_declarations, const_declarations):
        """
        Calculate if prerender is needed:
                - Only enable if view has async (@await or @fetch) AND output depends on @vars data
                - Dependency is considered true when:
                    + template/body references @vars variables
                    + declarations (useState/let/const) reference @vars variables
                    + sections/blocks are marked as useVars
                - If async exists but everything is independent from @vars data → prerender is disabled
        
        Args:
            raw_blade_code: Raw blade code BEFORE template_processor, so directives like @if/@foreach are visible
        """
        # If view doesn't have async, no prerender needed
        if not (has_await or has_fetch):
            return False
        
        # Extract vars/props names
        vars_names = self._extract_vars_names(vars_declaration)
        
        # Remove declaration directives before dependency scan to avoid
        # false positives from @vars(...) itself.
        scan_content = self._strip_declaration_directives(raw_blade_code)

        # Check if template body uses any vars in directives or bindings
        template_uses_vars = self._template_uses_vars(scan_content, vars_names)
        
        # Check if declarations use vars (useState/let/const may compute from vars)
        declarations_use_vars = self._declarations_use_vars(usestate_declarations, let_declarations, const_declarations, vars_names)
        
        # Check if any section uses vars for dynamic rendering
        sections_use_vars = any(section.get('useVars', False) for section in (sections_info or []))
        
        # Prerender is needed only when async output depends on @vars data
        return (template_uses_vars or 
                declarations_use_vars or 
            sections_use_vars)
    ...
    def _strip_declaration_directives(self, content):
        """Remove declaration-only directives before usage scanning."""
        if not content:
            return ''

        stripped = content
        # Single-line declaration directives that should not count as usage.
        stripped = re.sub(r'@vars\s*\([^)]*\)', '', stripped, flags=re.IGNORECASE)
        stripped = re.sub(r'@useState\s*\([^)]*\)', '', stripped, flags=re.IGNORECASE)
        stripped = re.sub(r'@states\s*\([^)]*\)', '', stripped, flags=re.IGNORECASE)
    ...
    def _generate_state_registration_lines(self, state_key, setter_name, initial_value):
        """Generate state registration code lines for a single state variable.
        Returns list of code lines to be added to wrapper_lines inline."""
        lines = []
        ...
        if self._is_typescript:
            ...
            lines.append(f"    const {setter_name} = (state) => {{")
        lines.append(f"        {state_key} = state;")
        lines.append(f"        {internal_register}(state);")
        lines.append(f"    }};")
        lines.append(f"    __STATE__.__.setters.{setter_name} = {setter_name};")
        lines.append(f"    __STATE__.__.setters.{state_key} = {setter_name};")
        # Add update$stateKey function
        update_func_name = f"update${state_key}"
        if self._is_typescript:
            lines.append(f"    const {update_func_name} = (value: any) => {{")
        else:
            lines.append(f"    const {update_func_name} = (value) => {{")
        lines.append(f"        if(__STATE__.__.canUpdateStateByKey){{")
        lines.append(f"            updateStateByKey('{state_key}', value);")
        lines.append(f"            {state_key} = value;")
        lines.append(f"        }}")
        lines.append(f"    }};")
        
        return lines

    def _extract_state_info(self, var):
        """Extract state key, setter name, and initial value from a useState variable.
        Returns dict with stateKey, setterName, initialValue or None."""
        names = var['names']
        value = var['value']
        
        if len(names) >= 2:
            state_key = names[0]
            setter_name = names[1]  # Keep user-declared setter name as is
    ...
    def _generate_wrapper_declarations(self, declarations):
        ...
        Preserves original declaration order - state registrations are emitted inline.
        ...
        for decl in declarations:
                ...
                for var in variables:
                    ...
                    if var.get('isDestructuring'):
                        ...
                        if var.get('isUseState'):
                            ...
                            # Generate state registration INLINE to preserve declaration order
                            state_info = self._extract_state_info(var)
                            if state_info:
                                state_declarations.append(state_info)
            ...
            elif decl_type in ('useState', 'states'):
                # Process @useState/@states directives - generate INLINE
                for var in variables:
    ...
    def _extract_declared_template_variables(self, variable_list, const_declarations, usestate_variables):
        """Extract variable names declared in wrapper scope for exec assignment safety."""
        declared = set(variable_list or [])
        declared |= set(usestate_variables or set())

        # Add names declared by @const (const declarations are not part of variable_list)
        for line in (const_declarations or '').split('\n'):
            s = line.strip()
            if not s.startswith('const '):
                continue

            m_obj = re.match(r'^const\s*\{([^}]*)\}\s*=.*$', s)
            if m_obj:
                for part in m_obj.group(1).split(','):
                    name = part.strip().split(':')[0].strip()
                    if re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', name):
                        declared.add(name)
                continue

            m_arr = re.match(r'^const\s*\[([^\]]*)\]\s*=.*$', s)
            if m_arr:
                for part in m_arr.group(1).split(','):
                    name = part.strip()
                    if re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', name):
                        declared.add(name)
                continue

            m_plain = re.match(r'^const\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=.*$', s)
            if m_plain:
                declared.add(m_plain.group(1))
