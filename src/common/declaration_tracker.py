...
from common.php_converter import php_to_js, convert_php_array_to_json
...
class DeclarationTracker:
    ...
    def _find_props_declarations(self, blade_code):
        """Find all @props declarations.
        Supports two formats:
        1. @props($a = 0, $c = 0) — same as @vars
        2. @props(['a' => '...', 'b' => 0]) — array format
        """
        pattern = r'@props\s*\('
        for match in re.finditer(pattern, blade_code):
            start_pos = match.end() - 1
            content, end_pos = extract_balanced_parentheses(blade_code, start_pos)
            if content is not None and content.strip():
                variables = self._parse_props_content(content.strip())
                self.declarations.append({
                    'type': 'props',
                    'position': match.start(),
                    'content': content.strip(),
    ...
    def _parse_props_content(self, content):
        """Parse @props content. Supports both standard and array format.
        Standard: @props($a = 0, $c = 0) → same parsing as @vars
        ...
        if content.startswith('[') and content.endswith(']'):
            return self._parse_props_array_format(content)
        
        # Standard format: same as @vars
        return self._parse_vars_content(content)
    
    def _parse_props_array_format(self, content):
        """Parse array format: @props(['key1' => value1, 'key2' => value2])
        Each key becomes a variable with its default value.
        """
        variables = []
    ...
    def _find_states_declarations(self, blade_code):
        """Find all @states declarations.
        Supports two formats:
        1. @states(['count' => 0, 'e' => $b]) — array format (like @useState array)
        2. @states($count = 0, $e = $b) — standard format (each becomes useState)
        """
        pattern = r'@states\s*\('
        for match in re.finditer(pattern, blade_code):
            start_pos = match.end() - 1
            content, end_pos = extract_balanced_parentheses(blade_code, start_pos)
            if content is not None and content.strip():
                variables = self._parse_states_content(content.strip())
                if variables:
                    self.declarations.append({
                        'type': 'states',
                        'position': match.start(),
    ...
    def _parse_states_content(self, content):
        """Parse @states content. Both formats produce useState variables.
        Array: @states(['count' => 0, 'e' => $b]) — same as @useState array
