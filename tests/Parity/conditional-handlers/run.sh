...
cat "$DIR/cases.jsonl" > "$WORK/cases.jsonl"; TOTAL=$(wc -l < "$WORK/cases.jsonl" | tr -d ' ')
...
if diff -u "$WORK/oracle.txt" "$WORK/subject.txt" > "$WORK/diff.txt"; then echo "✅ GOLDEN: khớp $TOTAL/$TOTAL"; exit 0; fi
echo "❌ GOLDEN LỆCH"; sed -n '1,50p' "$WORK/diff.txt"; exit 1
